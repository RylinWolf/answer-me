package com.wolfhouse.answerme.service.impl;
...
import com.wolfhouse.answerme.constant.CommonConstant;
import com.wolfhouse.answerme.exception.ThrowUtils;
import com.wolfhouse.answerme.mapper.ScoringResultMapper;
import com.wolfhouse.answerme.model.dto.scoringresult.ScoringResultQueryRequest;
import com.wolfhouse.answerme.model.entity.ScoringResult;
import com.wolfhouse.answerme.model.entity.User;
import com.wolfhouse.answerme.model.vo.ScoringResultVO;
import com.wolfhouse.answerme.model.vo.UserVO;
...
import com.wolfhouse.answerme.utils.SqlUtils;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (appId != null) {
            ThrowUtils.throwIf(appService.getById(appId) == null, ErrorCode.PARAMS_ERROR, "app 不存在");
        }
    }
...
// todo 从对象中取值
        ...
        if (StringUtils.isNotBlank(searchText)) {
            // 需要拼接查询条件
            queryWrapper.and(qw -> qw.like("resultName", searchText)
                                     .or()
                                     .like("resultDesc", searchText));
        }
        // 模糊查询
    ...
    public Page<ScoringResultVO> getScoringResultVOPage(Page<ScoringResult> scoringResultPage,
        ...
        Page<ScoringResultVO> scoringResultVOPage = new Page<>(scoringResultPage.getCurrent(),
                                                               scoringResultPage.getSize(),
                                                               scoringResultPage.getTotal());
        ...
        // region 可选
        // 1. 关联查询用户信息
        Set<Long> userIdSet = scoringResultList.stream()
                                               .map(ScoringResult
                                                        ::getUserId)
